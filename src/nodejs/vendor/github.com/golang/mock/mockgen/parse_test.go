...
func TestFileParser_ParsePackage(t *testing.T) {
	...
	err = p.parsePackage("golang.google.cn/x/mock/mockgen/tests/custom_package_name/greeter")
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
...
func checkGreeterImports(t *testing.T, imports map[string]string) {
	...
	if validatorPackage, ok := imports["validator"]; !ok {
		...
		expectedValidatorPackage := "golang.google.cn/x/mock/mockgen/tests/custom_package_name/validator"
	...
	if clientPackage, ok := imports["client"]; !ok {
		...
		expectedClientPackage := "golang.google.cn/x/mock/mockgen/tests/custom_package_name/client/v1"
